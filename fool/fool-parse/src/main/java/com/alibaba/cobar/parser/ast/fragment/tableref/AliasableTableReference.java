...
public abstract class AliasableTableReference implements TableReference {
    ...
    public String getAliasUnescapeUppercase() {
        return this.getAliasUnescape(true);
    }

    public String getAliasUnescape(boolean toUppercase) {
        if (alias == null || alias.length() <= 0)
            return alias;
        if (aliasUpEscape != null && toUppercase)
            return aliasUpEscape;
        if (aliasEscape != null && !toUppercase)
            return aliasEscape;
        ...
        switch (alias.charAt(0)) {
            case '`':
                if (toUppercase)
                    ...
                    return aliasEscape = Identifier.unescapeName(alias, false);

            case '\'':
                if (toUppercase)
                    return aliasUpEscape = LiteralString.getUnescapedString(alias.substring(1, alias.length() - 1), true);
                else
                    return aliasEscape = LiteralString.getUnescapedString(alias.substring(1, alias.length() - 1), false);
            ...
            case '_':
                ...
                if (ind >= 0) {
                    ...
                    if (toUppercase)
                        ...
                        return aliasEscape = st.getUnescapedString(false);
                }
            default:
                if (toUppercase) {
                    return aliasUpEscape = alias.toUpperCase();
                } else {
                    return aliasEscape = alias;
                }
        }
    }
