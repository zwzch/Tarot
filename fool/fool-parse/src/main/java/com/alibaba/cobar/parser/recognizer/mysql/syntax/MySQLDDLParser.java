...
public class MySQLDDLParser extends MySQLParser {
    ...
    private DDLCreateTableStatement createTable(boolean temp) throws SQLSyntaxErrorException {
        ...
        String subStr = String.valueOf(lexer.getSQL()).substring(lexer.getCurrentIndex());

        Identifier table = identifier();
        DDLCreateTableStatement stmt = new DDLCreateTableStatement(subStr, temp, ifNotExists, table);
    ...
    // column_definition:
    // data_type [NOT NULL | NULL] [DEFAULT default_value] [ON UPDATE on_update_value]
    ...
    private ColumnDefinition columnDefinition() throws SQLSyntaxErrorException {
        ...
        if(lexer.token() == KW_ON) {
            // 处理默认值的是 on update current_timestamp的情况
            if (lexer.nextToken() != KW_UPDATE || lexer.nextToken() != KW_CURRENT_TIMESTAMP) {
                throw new SQLSyntaxErrorException("default value on update current_timestamp is not wrong");
            }

            // TODO 只是解析了on update 后面的值得，但是没有存下来
            onUpdateVal = exprParser.expression();
        }
